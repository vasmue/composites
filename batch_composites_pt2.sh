#!/bin/bash
#SBATCH --job-name="composites-pt2"
#SBATCH --nodes=1
#SBATCH --ntasks=2
...
srun -n1 --exact --cpus-per-task=12 --gres=gpu:1 python make_composite.py 2019 ${var} &
srun -n1 --exact --cpus-per-task=12 --gres=gpu:1 python make_composite.py 2020 ${var} &
